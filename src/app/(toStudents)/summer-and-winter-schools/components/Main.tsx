  ...
  return (
    <section className='schools flex-center pb-20'>
      <div className='schools__wrapper wrapper--my w-full'>
        ...
        <div className='relative w-full lg:w-[90%] mx-auto aspect-[16/9] rounded-2xl overflow-hidden shadow-md mb-[30px] lg:mb-[40px]'>
          <Image
            src={summerAndWinterSchools.groupPhotos[4]}
            alt='Главное фото летней и зимней школы'
            fill
        ...
        <div className='flex flex-col gap-[40px] lg:gap-[50px] w-full lg:w-[80%] mx-auto mb-[60px]'>
          {summerAndWinterSchools.sections.map((section, index) => (
            <div
              key={index}
              className='bg-white p-[20px] lg:p-[30px] rounded-2xl shadow-sm border border-gray-100'
            >
                ...
                {section.content.map((block, bIndex) => {
                  ...
                  if (block.type === 'list') {
                    return (
                      <ul
                        key={bIndex}
                        className='flex flex-col gap-[10px] pl-[15px] lg:pl-[20px]'
                      >
                        {(block.value as string[]).map((item, iIndex) => (
                          <li
                            key={iIndex}
                            className="text-base lg:text-lg leading-relaxed relative before:content-['•'] before:absolute before:left-[-15px] before:text-blue-500 before:font-bold"
                          >
                            {item}
                          </li>
                        ))}
        ...
        <div className='w-full lg:w-[90%] mx-auto mb-[60px]'>
          <h3 className='wwtitle--bold text-center mb-[30px] lg:mb-[40px]'>
            Отзывы участников
          </h3>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-[20px] lg:gap-[30px]'>
            {summerAndWinterSchools.reviews.map((review, index) => (
              <div
                key={index}
                className='flex flex-col bg-blue-50 p-[20px] lg:p-[30px] rounded-2xl border border-blue-100 hover:shadow-md transition-shadow'
              >
                <div className='flex items-center gap-[15px] mb-[20px]'>
                  <div className='relative w-[60px] h-[60px] flex-shrink-0 rounded-full overflow-hidden shadow-sm border-2 border-white'>
                    <Image
                      ...
                      className='object-cover'
                      style={{
                        objectPosition: review.imagePosition || 'center',
                      }}
                      sizes='60px'
                    />
                  </div>
                ...
                <div className='relative'>
                  <span className='absolute top-[-10px] left-[-10px] text-4xl text-blue-200 font-serif'>
                    "
                  </span>
